...
def save_config_report(config_dict, work_dir):
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            "  - FIDT(num_classes={}, down_ratio={})\n".format(
                config_dict["model"]["num_classes"], config_dict["model"]["down_ratio"]
            )
        )
        f.write(
            "  - PointsToMask(radius=2, num_classes={}, squeeze=True, down_ratio=32)\n\n".format(
...
def main():
    ...
    train_dataset = CSVDataset(
        csv_file=train_csv,
        root_dir=train_root,
        albu_transforms=[A.VerticalFlip(p=0.5), A.Normalize(p=1.0)],
        end_transforms=[
