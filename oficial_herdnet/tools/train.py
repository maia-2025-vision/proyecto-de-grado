...
def _load_end_transforms(tr_cfg: DictConfig) -> list | None:
...
def _define_evaluator(
    ...
    if anno_type == "point":
        metrics = PointsMetrics(
            radius=cfg.training_settings.evaluator.threshold, num_classes=cfg.datasets.num_classes
        )
    elif anno_type == "bbox":
        metrics = BoxesMetrics(
            iou=cfg.training_settings.evaluator.threshold, num_classes=cfg.datasets.num_classes
        )
    elif anno_type == "image":
        metrics = ImageLevelMetrics(num_classes=cfg.datasets.num_classes)
    else:
        raise NotImplementedError
