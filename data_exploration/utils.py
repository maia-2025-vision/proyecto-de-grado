...
def plot_total_species_comparison(dataset_summaries, output_path):
    ...
    df = pd.DataFrame({
        name: {species: get_total_species_counts(summary).get(species, 0) for species in all_species}
...
def load_all_bbox_data(datasets, splits=["train", "val", "test"]):
...
def get_top_n_bboxes_in_area_range(bbox_data, area_min, area_max, n=5):
    """Retorna top n bboxes cuyo área está entre area_min y area_max, ordenados por área descendente."""
...
def plot_bbox_area_histogram_from_data(bbox_data, output_dir):
    """Genera un histograma de áreas relativas por clase separado por split, con colores distintos."""
...
def generate_split_table(split_info):
    ...
    lines.append(f"- **Tamaños únicos de imagen**:")
...
def analyze_area_statistics(bbox_data):
    ...
    image_sizes = df.groupby(['split', 'image_width', 'image_height']).size().reset_index(name='count')
